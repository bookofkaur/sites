...
assert.equal(d.getElementById('cl-price').value, '', 'price field stays blank for per-platform AI suggestions');
...
assert.equal(clCards[0].price, 119, 'eBay card uses the eBay suggested price');
assert.equal(clCards[1].price, 112, 'Mercari card uses the Mercari suggested price');
assert.equal(clCards[2].price, 105, 'Depop card uses the Depop suggested price');
